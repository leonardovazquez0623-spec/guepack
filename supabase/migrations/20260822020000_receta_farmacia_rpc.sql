...
-- El nuevo servicio "con_recoleccion" agrega 2 parámetros a la firma existente.
-- CREATE OR REPLACE no reemplaza una función cuando cambia la lista de
-- parámetros (crearía un segundo overload) — se elimina la firma anterior
-- explícitamente antes de crear la nueva versión.
DROP FUNCTION IF EXISTS public.crear_pedido_corporativo_atomico(
...
CREATE FUNCTION public.crear_pedido_corporativo_atomico(
  p_user_id TEXT,
  p_tenant_id BIGINT,
...
BEGIN
  ...
  END IF;
  IF p_tipo_servicio IS NULL OR p_tipo_servicio NOT IN ('directo', 'con_recoleccion') THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'TIPO_SERVICIO_INVALIDO';
  END IF;
  ...
  -- IVA es obligatorio para cualquier tipo de servicio.
  IF v_empresa.iva IS NULL THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'CONFIGURACION_CORPORATIVA_INVALIDA';
  END IF;
  v_iva := v_empresa.iva::NUMERIC;
  IF v_iva < 0 OR v_iva > 1 THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'CONFIGURACION_CORPORATIVA_INVALIDA';
  END IF;

  IF p_tipo_servicio = 'con_recoleccion' THEN
    -- Servicio "con recolección de receta": tarifa propia, nunca los campos
    -- de la tarifa por distancia del servicio directo.
    IF v_empresa.tarifa_receta IS NULL
       OR v_empresa.km_incluidos_receta IS NULL
       OR v_empresa.tarifa_km_receta IS NULL THEN
      RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'CONFIGURACION_RECETA_INVALIDA';
    END IF;
    IF v_empresa.tarifa_receta::NUMERIC < 0
       OR v_empresa.km_incluidos_receta::NUMERIC < 0
       OR v_empresa.tarifa_km_receta::NUMERIC < 0 THEN
      RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'CONFIGURACION_RECETA_INVALIDA';
    END IF;

    v_precio_base := v_empresa.tarifa_receta::NUMERIC;
    IF p_km_recorridos > v_empresa.km_incluidos_receta::NUMERIC THEN
      v_precio_base :=
        v_precio_base
        + (p_km_recorridos - v_empresa.km_incluidos_receta::NUMERIC)
          * v_empresa.tarifa_km_receta::NUMERIC;
    END IF;
    -- El cargo de paquete grande no aplica a este servicio, por diseño.
    v_cargo_paquete_grande := 0;
  ELSE
    ...
    IF v_empresa.tarifa_km IS NULL
       ...
       OR v_empresa.tarifa_base_extra IS NULL THEN
      RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'CONFIGURACION_CORPORATIVA_INVALIDA';
    END IF;
  ...
  INSERT INTO public.pedidos (
    ...
    tipo_servicio, receta_subestado
  )
  VALUES (
    ...
    p_tipo_servicio, p_receta_subestado
  )
...
COMMENT ON FUNCTION public.crear_pedido_corporativo_atomico(
  TEXT, BIGINT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DATE, TEXT, TEXT, NUMERIC,
  TEXT, TEXT, TEXT, TEXT, TEXT, NUMERIC, DOUBLE PRECISION, DOUBLE PRECISION,
  DOUBLE PRECISION, DOUBLE PRECISION, NUMERIC, TEXT, TEXT, TEXT
) IS 'Crea un pedido corporativo single-destino (directo o con_recoleccion de receta), recalcula la tarifa correspondiente y consume el cargo pendiente de forma atómica. Solo puede invocarse con service_role.';
