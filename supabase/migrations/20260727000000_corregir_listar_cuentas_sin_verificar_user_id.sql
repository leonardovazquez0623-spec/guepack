DROP FUNCTION IF EXISTS public.listar_cuentas_sin_verificar(BIGINT);
...
AS $$
  ...
  WHERE au.email_confirmed_at IS NULL
    AND (
      p_tenant_id IS NULL
      OR usuario.tenant_id = p_tenant_id
    )
    AND EXISTS (
      SELECT 1
...
REVOKE ALL
  ON FUNCTION public.listar_cuentas_sin_verificar(BIGINT)
  FROM PUBLIC;
