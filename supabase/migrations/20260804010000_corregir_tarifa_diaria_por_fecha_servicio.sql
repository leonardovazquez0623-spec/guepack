...
BEGIN
  ...
  /*
   * 4. Serializa la decisión "primer pedido de la fecha de servicio" por
   * tenant + empresa + p_fecha.
   *
   * La fecha que participa en el lock es la fecha efectiva del servicio,
   * no la fecha en que se creó el registro.
   *
   * Limitación conocida: multiparada diaria sigue en el flujo legado y no
   * participa en este advisory lock hasta que sea migrada en su propia fase.
   */
  ...
  /*
   * 5. Busca y bloquea el pedido que mantiene cubierta la tarifa para la
   * misma fecha efectiva de servicio.
   *
   * pedidos.fecha es TEXT. Antes de compararla contra p_fecha se exige:
   * - formato YYYY-MM-DD;
   * - conversión de ida y vuelta sin normalización;
   * - igualdad con la fecha solicitada.
   */
  SELECT *
  ...
  WHERE tenant_id = p_tenant_id
    ...
    AND to_date(fecha, 'YYYY-MM-DD') = p_fecha
...
) IS 'Crea un pedido corporativo simple de tarifa diaria, reserva atómicamente el primer cobro de la fecha efectiva del servicio y consume el cargo pendiente. Solo service_role.';
